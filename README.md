...
src/manager-544/
  ...
  pipeline/   email primit → OCR (Mistral) → analiză/clasificare (Mistral) → potrivire cu cererea → status
  inbound/    webhook Cloudflare Email Worker (R2 → MIME → atașamente → DB), adaptor campanii, webhook Resend (livrare)
  requests/   sesiuni + cereri, limita zilnică, șablon email, utilitare (extragere întrebare, termene, CSV)
  emails/     trimitere (Resend), URL semnat atașamente, interogări client
  questions/  generarea celor 5×10 întrebări strategice (Claude Haiku)
  chat/       asistent 544: guardrails pe pași, prompt, tool-uri (rag_search local + web_search), bucla Anthropic
...
3. Răspunsurile ajung prin Cloudflare Email Routing → worker → R2 → `POST /api/webhooks/cloudflare-email` → `pipeline/process-email`: OCR pe PDF, clasificare (`inregistrate`, `amanate`, `raspunse`, `intarziate`, `redirectionat`, `irelevant`), potrivire (thread → număr de înregistrare → expeditor), tranziție de status și termene (10 zile, 30 cu prelungire).
4. Cron-urile Vercel reprocesează emailurile rămase și marchează cererile depășite ca `delayed`.
...
| Anthropic | chat + întrebări (Claude Haiku 4.5, web search nativ) | `ANTHROPIC_API_KEY` |
| Mistral | OCR + clasificare emailuri | `MISTRAL_API_KEY`, opțional `MISTRAL_ANALYSIS_MODEL` (implicit `ministral-14b-latest`) |
| Resend | trimitere emailuri + webhook livrare | `RESEND_API_KEY`, `RESEND_WEBHOOK_SECRET` |
...
1. Rulează migrarea `016_refactor_hardening.sql` în Supabase (o singură dată, idempotentă).
...
4. Deploy; verifică `GET /api/chat-haiku` (health) și că `POST /api/emails/process` fără header răspunde 401.
