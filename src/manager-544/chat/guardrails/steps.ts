 ...
 * STEP_3 means "institution confirmed" (questions are generated by the wizard,
 * not in the chat), so only the institution marker can reach it.
 */
import {
  hasCompleteSummary,
...
function detectRawStep(history: HistoryMessage[]): Step {
  ...
  for (const msg of recent) {
    ...
    if (hasInstitutieMarker(msg.content)) return 'STEP_3';
  }
  return 'STEP_1';
}
